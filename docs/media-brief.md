...
# Плавний розворот циклу (заміна простого маятника)

Простий реверс дає різкий розворот: швидкість миттєво змінює знак,
і око це ловить. Заміряно на старому файлі — рух тримався на рівні
1.15–1.26 просто крізь точку розвороту, без жодного гальмування.

## Рецепт

1. **Ущільнити кадри інтерполяцією.** Без цього на сповільненні
   виходить «крок — завмер — крок»: семплюються дискретні кадри,
   і кожен крок має повну амплітуду. Заміряно: стрибок швидкості 1.27
   проти 0.42 у природній варіації кліпу.

   ```bash
   ffmpeg -i hero-raw.mp4 -t 4 \
     -vf "minterpolate=fps=120:mi_mode=mci:me_mode=bidir:mc_mode=obmc" \
     -fps_mode passthrough hi/h_%04d.png
   ```
   24 → 120 fps на 4 секундах: ~6 хвилин обробки, 480 кадрів.

2. **Побудувати послідовність зі smoothstep.** Похідна `u²(3−2u)`
   дорівнює нулю на обох кінцях, тож швидкість гасне сама.

   ```js
   const s = u => u*u*(3-2*u);
   const E = Math.round(LAST * 1.5 / 5);   // пік = 5 інтерп-кадрів/кадр
   const H = 19;                            // добір паузи до 1.0 с
   for (let k=0;k<E;k++) seq.push(Math.round(LAST * s(k/E)));      // вперед
   for (let k=0;k<H;k++) seq.push(LAST);                            // пауза
   for (let k=0;k<E;k++) seq.push(Math.round(LAST * (1-s(k/E))));   // назад
   for (let k=0;k<H;k++) seq.push(0);                               // пауза
   ```

   `E` рахується так, щоб пікова швидкість дорівнювала оригінальній.
   Пауза `H` добирається до потрібної: smoothstep уже дає ~5 нерухомих
   кадрів на кінці, решту додає `H`.

3. **Зібрати через concat demuxer** — без копіювання кадрів на диск:
   ...
   ```bash
   ffmpeg -f concat -safe 0 -i seq.txt -fps_mode cfr -r 24 \
...
Профіль швидкості = різниця між сусідніми кадрами:

```bash
ffmpeg -i hero-loop.mp4 -vf "tblend=all_mode=difference,signalstats,\
metadata=print:key=lavfi.signalstats.YAVG:file=motion.txt" -f null -
```

Має вийти дзвін: 0 → пік → 0 → пауза → 0 → пік → 0.
Якщо в точці розвороту значення не падає до нуля — згладжування
не спрацювало.

## Отримані параметри

| | Маятник | smoothstep + mci | smootherstep + blend |
|---|---|---|---|
| Тривалість | 7.92 с | 13.63 с | **17.04 с** |
| Пауза на краю | немає | 1.00 с | **1.71 с** (сприймана 2.2 с) |
| Швидкість у розвороті | 1.21 | 0.00 | 0.00 |
| Ривків понад 0.3 | — | 65 | **29** |
| Середній модуль зміни | — | 0.156 | **0.110** |
| Стик циклу | 0.34 | 0.05 | **0.001** |
| MP4 / WebM | 574/372 KB | 788/466 KB | 855/521 KB |

## Дві пастки, на які я натрапив

**1. `mi_mode=mci` розкладає кадри нерівномірно.** На дуже повільному русі
motion-compensated інтерполяція для частини пар знаходить нульовий рух і
дублює кадр, а потім «наздоганяє». У зоні гальмування це давало
`0.01 → 0.59 → 0.01` замість плавного згасання — тобто судому, гіршу за
початкову різкість.

Використовувати **`mi_mode=blend`**: кроки рівні за побудовою, і обробка
займає 18 секунд замість 6 хвилин.

```bash
ffmpeg -i hero-raw.mp4 -t 4 -vf "minterpolate=fps=120:mi_mode=blend" \
  -fps_mode passthrough hb/b_%04d.png
```

Розмиття від блендингу не заважає: при піковій швидкості семплюється
кожен пʼятий кадр, тобто рівно оригінальні. Проміжні показуються лише
на сповільненні, де сусідні кадри майже не відрізняються.

**2. `smoothstep` замало, щоб прибрати «ефект відбиття».** Її друга
похідна на кінцях не нульова, тож момент зупинки все одно читається як
удар об стіну. `smootherstep` — `u³(6u²−15u+10)` — гасить і другу
похідну: рух розчиняється, а не приїжджає в стоп.

```js
const smoother = u => u*u*u*(u*(u*6-15)+10);   // пік похідної 1.875
